...
export default {
  ...
  setup() {
    ...
    const fetchData = async () => {
        ...
        axios.get(`https://juanutrisyon.info/api/foods/${food_slug}`)
            .then(async (res) => {
                const age = res.data.age;
               
                //
                // calories
                const energy_intake_res = await axios.get(`https://juanutrisyon.info/api/reni-energy-intake?age=${age}`);
                calorie_req_in_kcal.value = energy_intake_res.data.male_energy_req_in_kcal;
                
                // fiber
                const fiber_intake_res = await axios.get(`https://juanutrisyon.info/api/reni-recommended-macro-intake?age=${age}`);
                
                // macros
                const macro_intake_res = await axios.get(`https://juanutrisyon.info/api/reni-macro-intake-distribution?age=${age}`);
            
                // vitamins
                const vitamin_intake_res = await axios.get(`https://juanutrisyon.info/api/reni-recommended-vitamin-intake?age=${age}`);
                
                // minerals
                const mineral_intake_res = await axios.get(`https://juanutrisyon.info/api/reni-recommended-mineral-intake?age=${age}`);

                // fda daily values (for adults)
                const fda_daily_nutrient_values_res = await axios.get(`https://juanutrisyon.info/api/fda-daily-nutrient-values`);
